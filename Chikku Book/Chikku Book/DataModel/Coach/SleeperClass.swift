...
struct SleeperClass:Coach{
    let coachNumber: Int
    let coachTypeDetails: CoachTypeDetails
    lazy private(set) public var seats: [Seat] = createSeats()
    private let seatTypes : [SeatType] = [.Lower,.Middle,.Upper,.SideLower,.SideUpper]
    
    init(coachNumber: Int){
        self.coachNumber = coachNumber
        self.coachTypeDetails = DBManager.getInstance().getCoachTypeDetails(coachType: .SleeperClass)!
        self.seats = createSeats()
    }
    
    func createSeats() -> [Seat] {//get seats
        var seats = [Seat]()
            ...
            for  columnCount in 0..<totalColumns{
                ...
                if(rowCount % 2 == 0) {
                    ...
                    if(columnCount == totalColumns-1) {
                        ...
                        //print("Seat Number \(seatNumberCount-1)")
